...
class ValueFailure<T> with _$ValueFailure<T> {
  ...
  const factory ValueFailure.invalidKtp({
    required T? failedValue,
    required int shouldKtpLength,
  }) = InvalidKtp<T>;

  const factory ValueFailure.invalidPin({
    required T? failedValue,
    required int shouldPinLength,
  }) = InvalidPin<T>;
  ...
  const factory ValueFailure.startDateOrEndDateShouldNotNull({required T? failedValue}) =
  StartDateOrEndDateShouldNotNull<T>;
  const factory ValueFailure.endDateShouldGreaterThanStartDate({required T? failedValue}) =
