            ...
            body: ListView.builder(
                itemBuilder: (context, index) {
                  return GestureDetector(
                    onTap: () {
                      Modular.get<MealBloc>().add(MealEvent.categorySelected(state.categories[index]));
                      Modular.to.pushNamed(AppModule.MEAL);
                    },
                    child: Card(
                      ...
                      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        SizedBox(
                                ...
                                child: MyImage(path: state.categories[index].strCategoryThumb))
                          ...
                          child: Column(
                            ...
                            children: [
                              Shimmer.fromColors(
                                  ...
                                  child: Text(state.categories[index].strCategory, style: const TextStyle(color: Colors.black),)),
                              Shimmer.fromColors(
                                baseColor: basicShimmerColor,
                                highlightColor: highlightShimmerColor,
                                enabled: state.isLoading,
                                child: Text(
                                  '${state.categories[index].strCategoryDescription.split('.')[0]}.',
                                  maxLines: 6,
                                  softWrap: true,
                                  overflow: TextOverflow.ellipsis,
                                  style: const TextStyle().copyWith(fontSize: 10, color: Colors.black),
                                ),
                              )
                            ],
                          ).paddingAll(defaultMargin(context)),
